...
public class GameScreen implements Screen {
    ...
    public void render(float delta) {
        ...
        for (Cangaroo cangaroo : peanutButters) {
            cangaroo.act(delta); // Actualiza la posición del objeto

            if (player.getBounds().overlaps(cangaroo.getBounds()) && !pilled && !dead) {
                dead = true;
                break;
            }
        }
    ...
    private void shoot() {
        ...
        cangaroo.setX(800);
        cangaroo.setY(holey);
        cangaroo.setAssetManager(game.manager);
        peanutButters.add(cangaroo);
        stage.addActor(cangaroo);
    }
